...
def build_cmd(step, dataset, ttl, csv=None, seed=None):
    ...
    if step == "train": return ["python3", "-u", "src/train.py", "--ttl", ttl, "--emb", f"data/processed/{dataset}/entity_embeddings.npy", "--entity2id", f"data/processed/{dataset}/entity2id.json", "--case_split", f"data/processed/{dataset}/case_split.json", "--save_best_val", f"data/processed/{dataset}/best_val.pt"]
    if step == "evaluate": return ["python3", "-u", "src/evaluate.py", "--best", f"data/processed/{dataset}/best_val.pt", "--vocabs", f"data/processed/{dataset}/best_val_vocabs.json", "--entity2id", f"data/processed/{dataset}/entity2id.json", "--output", f"data/processed/{dataset}/evaluation.txt"]
